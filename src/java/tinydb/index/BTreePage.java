...
public abstract class BTreePage implements Page {
	...
	/**
	  ...
	  BTreeInternalPage 的格式是一组标头字节，指示正在使用的页面的槽位，一些条目槽位，
	  以及父指针的额外字节，一个额外的子指针（具有 m 个条目的节点有 m+1指向子页面的指针），
	  以及所有子页面的类别（叶子页面或内部页面）
	   Specifically, the number of entries is equal to: <p>
	           floor((BufferPool.getPageSize()*8 - extra bytes*8) / (entry size * 8 + 1))
	  <p> 其中条目大小是此索引节点中条目的大小
	...
	/**
	 * 生成与空 BTreePage 对应的字节数组的 static 方法
	 * 用于向文件添加新的空白页面。
	 * 将此方法的结果传递给 BTreeInternalPage 或 BTreeLeafPage 构造函数将创建一个没有有效条目的 BTreePage
	 *
