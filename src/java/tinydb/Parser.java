...
public class Parser {
    ...
    void processExpression(TransactionId tid, ZExpression wx, LogicalPlan lp)
            throws tinydb.ParsingException, IOException, ParseException {
    ...
    public Query handleQueryStatement(ZQuery s, TransactionId tId)
        ...
        if (physicalPlan != null) {
            Class<?> c;
            try {
                c = Class.forName("tinydb.optimizer.OperatorCardinality");
                ...
                c = Class.forName("tinydb.optimizer.QueryPlanVisualizer");
                m = c.getMethod(
    ...
    public Query handleDeleteStatement(ZDelete s, TransactionId tid)
            throws
            tinydb.ParsingException, IOException, ParseException {
        int id;
        try {
    ...
    public void handleTransactStatement(ZTransactStmt s)
            throws IOException,
            tinydb.ParsingException, TransactionAbortedException, DbException {
    ...
    public LogicalPlan generateLogicalPlan(TransactionId tid, String s)
            throws tinydb.ParsingException, IOException {
    ...
    public void processNextStatement(String s) throws TransactionAbortedException {
        processNextStatement(new ByteArrayInputStream(s.getBytes(StandardCharsets.UTF_8)));
    }

    public void processNextStatement(InputStream is) throws TransactionAbortedException {
        try {
    ...
    public static void main(String[] argv) throws IOException, TransactionAbortedException {
    ...
    protected void start(String[] argv) throws IOException, TransactionAbortedException {
        ...
        if (!interactive) {
            try {
                ...
                System.in.read();
                this.shutdown();
            } catch (FileNotFoundException | TransactionAbortedException e) {
